...
function doGit (commit, tag, push, ver, custom_config) {
  // 提交package.json / outFiles / CHANGLOG.md
  const msg = `"chore(release): ${ver}"`
  if (commit) {
...
async function start () {
  ...
  const { new_version, commit, tag, push } = ans
  if (new_version !== 'cancel' && new_version !== old_version) {
    ...
    doGit(commit, tag, push, new_version, custom_config)
  }
