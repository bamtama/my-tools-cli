...
class FtpCore {
  ...
  async clearRemote () {
    try {
      ...
      const reslist = await utils.asyncPool(5, filelist, this.delFileRemote(file))
      ...
      this.ftp.list(path, (err, list) => {
        if (err) {
          ...
          if (list && list.length > 0) {
            ...
            if (dirs.length > 0) {
              return utils.asyncPool(5, filelist, this.findFileRemoteWaitForDel(dir.d_filepath, reslist)).finally(() => {
                resolve()
              })
            } else {
              resolve()
  ...
  async uploadingFile (filelist) {
    ...
    const reslist = await utils.asyncPool(5, filelist, this.tickUpload.bind(this))
